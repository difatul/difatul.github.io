            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php
                  try{
                    $query=$conn->prepare("SELECT pelanggan.* FROM pelanggan WHERE 1");
                    $query->execute();
                    while($data=$query->fetch(PDO::FETCH_ASSOC)){

                    $tagihan = $conn->prepare("SELECT count(*), tagihan.jumlahmeter FROM tagihan, pelanggan WHERE tagihan.idpel=pelanggan.idpel AND tagihan.idpel='$data[idpel]' AND tagihan.status='Belum Terbayar'");
                    $tagihan->execute();
                    $tagihan2 = $tagihan->fetch(PDO::FETCH_NUM);
                    $tagihan1=$tagihan2[0]; ?>
                <tr>
                  <td><?php echo $data['idpel'];?></td>
                  <td><?php echo $data['nm_pel'];?></td>
                  <td><?php echo $tagihan1." Bulan";?></td>
                  <td>
                    <div align="center">
                      <a data-toggle="tooltip" data-placement="top" title="Detail" style="margin-right:5px" class="btn btn-success btn-sm" href="?mod=tagihan/detail&idpel=<?php echo $data['idpel'];?>">
